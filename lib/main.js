        ...
        case 'text/css':
          readCSS(url.format({
            ...
            if (options.production) {
              ...
              body.replace(/(background\-image|background|content)\:[^;]*url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, attribute, url) {
            ...
            async.parallel(imagesToUpload, function() {
              body = body.replace(/(background\-image|background|content)\:[^;]*url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, attribute, url) {
                return attribute + ':url(' + images[url] + ')';
              });
